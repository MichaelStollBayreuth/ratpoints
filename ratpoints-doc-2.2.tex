...
bit array to~$x$ (a real number; this is what decides~$n$), and the number of
primes added to~$n$ to obtain~$N$ to~$e$. The mnemonic is that \verb+-r+ and
...
are \texttt{RATPOINTS\_SURVIVORS\_PER\_ARRAY}
...
\texttt{survivors\_per\_array} and \texttt{sp2\_extra} hold the two
...
It takes a few minutes, wants an otherwise idle machine, and must not be run
under \texttt{make -j}. It is deliberately not part of \texttt{make all}.
