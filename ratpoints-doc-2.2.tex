...
\newcommand{\rpversion}{2.2.4} % don't forget the verbatim stuff!
...
\begin{verbatim}
> tar xzf ratpoints-2.2.4.tar.gz
\end{verbatim}
...
memory bandwidth rather than by arithmetic, so wider registers do not
automatically pay off, and some Intel server processors reduce their clock
...
> make test1once
\end{verbatim}
runs the same curves again, but with the fields of the argument structure
set once before the loop over the curves instead of once per curve, as a
program using the library may do (Section~\ref{sec:library});
\texttt{rptest} prints a line whenever the library has changed a field it
must leave alone, and the output is compared against \texttt{testbase} as
before.
...
one or more for each of the bugs fixed in version 2.2.4, and compares the
output against \texttt{testbase3}.
...
16, forbidden divisors of every kind, every option, restricted denominator
...
suite was written for version 2.3 and adapted; the reference was checked
independently of the sieve by \texttt{verify-test4.py}, which reads it
and, for every invocation that prints points, searches every coprime pair
$(a, b)$ in the range by brute force, in arbitrary precision (Python~3);
and version 2.3 prints the same points on the whole first part of the
script.
...
512; \texttt{RATPOINTS\_CHUNK=1}, one register in the first phase;
\texttt{USE\_LONG\_IN\_PHASE\_2}, the second phase one word at a time;
and \texttt{PRIME\_SIZE} 8, the larger prime table.  Each is built in a
directory of its own, so that the build in the working directory is left
alone (\texttt{test4-configs.sh}); the points found must not depend on
any of these, so every build is compared against the same
\texttt{testbase4}.  About a quarter of a minute.  The script exits with
status 1 when an output differs from the reference and with 2 when a build
failed; so does \texttt{coverage.sh} below.
...
\texttt{make test} but \texttt{test2} and \texttt{timing}) and prints,
for each source file, the lines executed and the branches taken at least
once (\texttt{coverage.sh}); the annotated sources are left in that
directory.
...
> make test
\end{verbatim}
does all of the above, each of them whatever the earlier ones did, and
fails at the end if any of them failed, so that a script can tell whether
the build passed.
...
\begin{itemize}
  ...
  \item \texttt{main.c, rptest.c, rpapi.c} --- the source code for the
        ...
        respectively
  \item \texttt{testdata.h} --- the curves used by \texttt{make test1}
  \item \texttt{testbase}, \texttt{testbase2}, \texttt{testbase3},
        \texttt{testbase4}, \texttt{testbase-api} --- the output the
        test runs are expected to produce
...
and are not of interest here. All the fields shown are inputs. Some of
them the program writes as well, and this is the complete list: the
coefficient array and \texttt{degree}, which on return describe the
polynomial the program worked with; \texttt{num\_inter} and
\texttt{domain}, which on return describe the region it searched; the
fields that ask for a default value when negative or non-positive
(\texttt{sp1}, \texttt{sp2}, \texttt{num\_primes}, \texttt{max\_forbidden},
\texttt{sturm}, \texttt{b\_low}, \texttt{b\_high}, \texttt{array\_size}),
into which it writes the value it used; and the flag bits that report on
the run. A caller who fills the structure once and then loops over curves
must therefore set before every call whatever it wants chosen afresh.
(Up to version 2.2.3 the program also wrote the number of forbidden
divisors it had found into \texttt{max\_forbidden}, and its own decision
not to reverse the polynomial into \texttt{flags}, which broke exactly
such loops; it no longer does.)
...
\texttt{cof}, \texttt{degree} and \texttt{num\_inter} must be set for every
curve, and so must the fields that are to be chosen afresh (see above); the
others keep their values. \texttt{rptest.c} with the \verb+-O+ option does
exactly this, and \texttt{make test1once} checks that the points found are
the same as with every field set once per curve.
...
(of 32 or 64 bits, as the case may be, or 128, 256 or 512 bits when SSE/AVX
...
\textbf{Version 2.2.4} was released September 13, 2026. It fixes bugs that a
review of the code turned up. A polynomial of degree~1 --- given as such,
...
its arrays and crash; this affects library users as well. When the
positivity region of~$f$ did not meet the search domain, the program
...
of the given curve. When no residue class of the denominator modulo~16
...
And several input fields of \texttt{ratpoints\_args} were used as scratch
...
\texttt{ratpoints} per bug against a reference. Also fixed: four left shifts of negative values
