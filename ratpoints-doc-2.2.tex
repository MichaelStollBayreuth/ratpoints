...
whenever the parameters are retuned (see Section~\ref{sec:tuning}), since a
setting that suits one of them can be a poor choice for the other.
...
\begin{itemize}
  ...
  \item \texttt{testdata.h, testdata-many.h, testbase, testbase-many,
        testbase2} --- data for the test runs
...
expected number of survivors per bit array falls below a constant --- almost
independently of the curve, even though the resulting \texttt{sp1} itself
ranges from $8$ to~$30$. That constant is
\texttt{RATPOINTS\_SURVIVORS\_PER\_ARRAY} in \texttt{ratpoints.h}; it is a
property of the machine rather than of the curve. Then \texttt{sp2} is set to
...
worth some $7\%$ on its own. A third constant,
\texttt{RATPOINTS\_PHASE2\_GROUP}, can make that loop test several bit arrays
at once by or-ing them together; that used to pay when \texttt{sp1} was fixed
and the proportion of surviving bit arrays therefore varied a great deal, but
now that the proportion is held down to a couple of per cent it no longer does,
and the default is~$1$. Setting it to~$0$ removes the skip loop altogether,
...
that on your machine, build with, say,
\begin{verbatim}
> make test1 test1many CCFLAGS='-DRATPOINTS_SURVIVORS_PER_ARRAY=0.03 \
                                -DRATPOINTS_SP2_EXTRA=5 \
                                -DRATPOINTS_PHASE2_GROUP=1'
\end{verbatim}
for a few values of each. The threshold matters considerably more than the
offset, which at large height bounds hardly matters at all. The threshold and
the group size should be tuned together, since the best group size depends on
how many bit arrays the first stage leaves alive, which is what the threshold
sets. Use both tests:
they cover the two regimes that occur in practice, and a value that suits one
of them can be a poor choice for the other.
