...
before relying on it. Be aware that AVX-512 is not a safe bet for a
speed-up: the sieving loop is limited by memory bandwidth rather than by
arithmetic, and some Intel server processors reduce their clock frequency
when 512-bit instructions are used. It is worth timing the 256-bit and the
512-bit version against each other on your machine.
...
genuine 512-bit instructions, so a first run on real hardware is still
wanted.
