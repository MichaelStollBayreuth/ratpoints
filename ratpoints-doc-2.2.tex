...
and are not of interest here. All the fields shown are inputs, and since
version 2.2.4 they come back from a call as they went in, with two
...
that report on the run. (Up to version 2.2.3 the program wrote some of the
values it chose or normalised into the input fields, so that a caller who
fills the structure once and then loops over curves had to reset them
before every call.)
...
calls, since version 2.2.4. \texttt{rptest.c} with the \verb+-O+ option does
...
of the others, which loses points. The input fields now come back as they
went in; \texttt{make test1once} runs the test curves that way, and
\texttt{make test3} runs one invocation of \texttt{ratpoints} per bug
against a reference. Also fixed: four left shifts of negative values
