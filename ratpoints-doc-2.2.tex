...
suite was written (by Claude) for version 3.0 and adapted; the reference was checked
independently of the sieve.
...
compile-time switches: the register widths~64, 128 (both variants) and~512;
\texttt{RATPOINTS\_CHUNK=1}, one register in the first phase;
...
any of these, so every build is compared against the same~\texttt{testbase4}.
...
and~\texttt{timing}) and prints, for each source file, the lines executed
and the branches taken at least once (\texttt{coverage.sh});
the annotated sources are left in that directory.
...
\begin{itemize}
  ...
  \item \texttt{verify-test4.py} --- the brute-force check of~\texttt{testbase4}
