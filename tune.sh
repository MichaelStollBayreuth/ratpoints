...
# there.
#
# The offset is the third constant's business too.  It is the number of
# extra primes an arbitrarily long run wants, and a run of U numerator words
# gets sp2_extra/(1 + U0/U) of them, so the ladder for it is wide and the
# value that suits a short run is not the value that suits a long one.  That is the point: U0 is what reconciles the
...
# stage runs at all on a curve (ratpoints.h, RATPOINTS_SP3_PER_DENOM).  The
