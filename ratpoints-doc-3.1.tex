...
\newcommand{\rpversion}{3.1.0} % don't forget the verbatim stuff!
...
bugs. This has resulted in version~3.0.0, which includes
...
The current version~3.1.0 adds the option to run the sieve on several
threads (Sections~\ref{sec:threads} and~\ref{sec:library}).
...
\begin{verbatim}
> tar xzf ratpoints-3.1.0.tar.gz
\end{verbatim}
...
\subsection{Build the program and library}\label{sec:build}
...
The sieve can run on several threads (option~\verb+-t+,
Section~\ref{sec:threads}; field \texttt{num\_threads},
Section~\ref{sec:library}), for which the library uses POSIX threads and
is compiled with \verb+-pthread+. On a system without them,
\begin{verbatim}
> make THREADS=0
\end{verbatim}
builds the library without threads (\verb+-DRATPOINTS_NO_THREADS+); the
option and the field are then accepted and ignored, and the sieve runs on
the calling thread. At most $33$ threads sieve at once,
whatever is asked for: the correction of the sieving parameters during a
run (Section~\ref{sec:adapt}) takes effect $32$ blocks of the loop over
the denominators after it is decided, which is what lets that many blocks
be in progress at the same time, and the library creates no threads
beyond that. On a machine with more cores, build with a larger lag, as in
\begin{verbatim}
> make all CCFLAGS=-DRP_ADAPT_LAG=64
\end{verbatim}
for up to $65$ threads (the \texttt{Makefile} notices the changed flags
and rebuilds what depends on them). The correction then comes later in the run, by a
few per cent of it; the points found are the same, but the reports of
\verb+-v+ on a long run may differ from those of a build with the
default lag, as they may between any two builds.
...
% into general registers. That is worth about 30\% of the second stage of the
...
CCFLAGS1 = ${CCFLAGS512}
\end{verbatim}
which requires a CPU with AVX512F capability. I have no such CPU available
and can test this variant only in the emulation described below;
\textbf{Drew Sutherland} has run the complete test suite of version~3.0.0 on
an AMD Ryzen~9 9950X3D (Zen~5 architecture), where it passes. It is worth
timing the 256-bit and the 512-bit version against each other on your
machine: the sieving loop is limited by how fast it loads bit arrays from the
first-level cache and by how much of that cache the sieve tables occupy,
rather than by arithmetic, so wider registers do not automatically pay off,
and some Intel server processors reduce their clock frequency when 512-bit
instructions are used. On the Zen~5 machine, with the constants of
\texttt{make tune} (Section~\ref{sec:retune}) in both builds, the 512-bit
version was 14\% faster than the 256-bit one on a long run (the curve of
\texttt{make timing}, height bound $4\cdot10^5$), equal on the random test
curves at~$16383$ and on the sextic of \texttt{make test2} at~$10^6$, and
8\% slower on the point-rich curves at~$16383$, where building the sieve
tables, twice as large with 512-bit registers, is a large share of the run.
Wider registers thus help long runs on curves that need few sieving moduli,
and not short ones. Run \texttt{make tune} after choosing the width: on that
machine it gained 6 to 7\% at either width over the constants compiled in,
which were measured on the author's laptop, and it found the same values at
both widths.
...
\vfill\pagebreak % manual formatting
...
When this was written the tests executed $99.5\%$ of the lines
...
against states no caller can produce, the paths that a failure to create a
thread or to get memory would take, arms that other compile-time settings
...
> make testthreads
\end{verbatim}
runs \texttt{rptest}, \texttt{rptest-many} and \texttt{rptest-degrees} with
two to five sieving threads and \texttt{test3} and \texttt{test4} with
three and seven, against the same references as before, since what the
program prints must not depend on the number of threads; then
\texttt{test4-threads.sh} builds the library three more times with the
blocks into which the loop over the denominators is cut
(Section~\ref{sec:threads}) fixed at 1, 3 and 17 steps, and runs
\texttt{test4} and \texttt{rptest} on each with three and six threads,
since the output must not depend on where the blocks end either.
About a minute.

\begin{verbatim}
> make tsan
\end{verbatim}
builds the library and the test programs in \texttt{build-tsan/} with
\texttt{gcc}'s ThreadSanitizer and runs the threaded tests on them
(\texttt{tsan.sh}, which has the compiler flags): every output must match
its reference, and the sanitizer must not report a data race. A few minutes.
...
This may be useful for comparisons. To time it with four threads, say
\begin{verbatim}
> make timing RPOPTS='-t 4'
\end{verbatim}
...
change to the sieve rather than for checking that the build works. Neither
are \texttt{testthreads} and \texttt{tsan}, which take longer than the rest
together.
...
\texttt{gmp} (GNU multi-precision) library~\cite{gmp}. Sieving on several
threads needs POSIX threads, which every Unix-like system has;
\texttt{make THREADS=0} builds without them (Section~\ref{sec:build}).
...
\begin{itemize}
  ...
  \item \texttt{test3.sh}, \texttt{test4.sh} --- the scripts that
        ...
        executable, as have the next five
  \item \texttt{test4-configs.sh}, \texttt{coverage.sh} --- what
        \texttt{make test4configs} and \texttt{make coverage} run
  \item \texttt{test4-threads.sh}, \texttt{tsan.sh} --- the second half of
        \texttt{make testthreads}, and what \texttt{make tsan} runs
  ...
  \item \texttt{ratpoints-doc-3.1.tex} --- the source of this documentation
        (\texttt{make all} or \texttt{make doc} produce the PDF file)
  \item \texttt{README.md} --- a short description and an overview of what
...
\subsection{Using several threads}\label{sec:threads}

\strut\quad\verb+-t+ $n$ \\
runs the sieve on $n$ threads; \verb+-t 0+ uses as many as the machine has
processors. By default the program uses one thread, its own. With
\verb+-t+~$n$ for $n \ge 2$, $n$ further threads do the sieving and the exact
tests, while the program's own thread hands out the work and prints the
points. The loop over the denominators is cut into blocks of consecutive
denominators, whose length depends only on the search and not on the
number of threads; the threads take the blocks in order, and the points
of a block are printed only when every earlier block has been printed.
So the output is the same, line for line, however many threads are used,
including the reports of~\verb+-v+ and what \verb+-1+ finds: the sieving
parameters are chosen and revised from the same counts on the same
schedule, and the first point is the first point of the sequential
program, not the first one found in time. At most $33$ threads sieve at
once, so \verb+-t 0+ on a machine with more processors uses~$33$; a
build for more cores is described in Section~\ref{sec:build}. On a
processor with hyperthreading, one thread per physical core is the better
choice, and \verb+-t 0+ counts hardware threads: on an Intel i7-13700K
with eight performance and eight efficiency cores, 16 threads ran the two
long test suites $8$ and $9$ times faster than one thread, and 24 threads,
one per hardware thread, $7$ and $8$ times.

Threads pay for long runs. On a 16-core AMD Ryzen~9 9950X3D, a search
with a height bound of $4\cdot10^5$ (the curve of \texttt{make timing})
takes $1.50$~seconds on one thread and $0.80$, $0.45$, $0.24$ and $0.16$
on 2, 4, 8 and~16 (measured by \textbf{Drew Sutherland}); the two long
test suites, 1008 random curves and 30 point-rich curves at height bound
$2\cdot10^5$, run $5.7$ and $7.9$ times faster on 16 threads than on one,
since every curve has a set-up that one thread does. On the author's
laptop, whose power budget lowers the clock as more cores work, the same
search takes $1.9$~seconds on one thread and $1.0$, $0.7$ and $0.4$ on 2,
4 and~10; the sieve's work is divided without loss (measured in processor
cycles, two threads spend what one does), so what the elapsed time shows
beyond that is the machine. At a small height bound the set-up of a
search is a large share of the run and there is little to gain; many
curves at a small height bound are better searched by independent
searches running in parallel (Section~\ref{sec:library}).
...
\texttt{num\_threads} says how many threads sieve (option~\verb+-t+,
Section~\ref{sec:threads}): $1$ or~$0$, the calling thread alone; $n \ge 2$,
$n$ threads besides the calling one, which hands out the work and delivers
the points; negative, as many threads as there are online processors. The
threads are created at the first call that asks for them and kept, with
their working memory, until \texttt{find\_points\_clear}; so a program
that searches many curves should use the \texttt{find\_points\_init},
\texttt{find\_points\_work}, \texttt{find\_points\_clear} sequence
described below rather than \texttt{find\_points}, which creates and ends
them on every call. A call with another value of \texttt{num\_threads}
replaces them. If the threads cannot be created, the calling thread sieves
alone. No more than $33$ threads are created, whatever the field says,
unless the library is built for more (Section~\ref{sec:build}). A library
built without threads (\texttt{make THREADS=0}) ignores the field.
...
Whatever \texttt{num\_threads} says, \texttt{proc} is called from the
thread that called \texttt{find\_points}, never from a sieving thread, and
for the same points in the same order as a single thread finds them; the
return value of \texttt{find\_points} and the point at which \texttt{*quit}
takes effect are the same as well. So \texttt{proc} and \texttt{info} need
no protection against concurrent calls. A sieving thread keeps the points
it finds until every earlier block of the search has been delivered; if it
cannot get the memory for that, the search fails with the return value
\texttt{RATPOINTS\_NO\_MEMORY} rather than deliver fewer points than it
found.
...
int main(int argc, char *argv[])
{
  ...
  long num_threads   = 1;  /* sieve on this thread; see -t */
  long b_low         = 1;
  long b_high        = height;
...
Searches on different \texttt{ratpoints\_args} structures are independent
of each other: the library keeps no state outside the structure (its
tables of primes and residues are constant), so a program may run several
searches at the same time on threads of its own, each with its own
structure, callback and \texttt{info}, and each with a
\texttt{num\_threads} of its own. For many searches at a small height
bound this is the better way to use a machine's cores
(Section~\ref{sec:threads}).
...
The compiled-in values are the laptop's, with one exception: two faster
machines, an Intel i7-13700K and an AMD Ryzen~9 9950X3D, both found the
threshold $0.002$ better than the laptop's $0.003$ by $2$ to $3.6\%$ on the
two test sets, and the lower value costs nothing on the laptop, so it is
the one compiled in; on the i7-13700K it makes the random test curves
$4.6\%$ faster at $16383$ and $2.2\%$ at $2\cdot10^5$ and changes nothing
on the point-rich ones. The offset is where machines and height bounds
disagree: on the i7-13700K, \texttt{make tune} moved it from $11$ to~$4$,
\texttt{make tunehigh} back to~$6$, and even that gained $2$ to $3\%$ on
three of the four test sets while losing $4.6\%$ on the point-rich curves
at $2\cdot10^5$. So the compiled-in offset stays, and a retuned one is
worth having only if your runs look like the sets it was measured on.
...
\textbf{Version 3.1.0} was released September 24, 2026. The sieve can run on
several threads.
\begin{itemize}
  \item \emph{Threads.} The option \verb+-t+~$n$ and the field
        \texttt{num\_threads} of \texttt{ratpoints\_args} run the sieve
        and the exact tests on $n$ threads besides the calling one, which
        hands out the work and delivers the points
        (Sections~\ref{sec:threads} and~\ref{sec:library}). The loop over
        the denominators is cut into blocks whose length depends on the
        search alone, the threads take the blocks in order, and the points
        of a block are delivered after those of every earlier block: what
        the program prints, what the callback receives and in which order,
        and where \texttt{*quit} takes effect do not depend on the number
        of threads, and neither do the sieving parameters chosen and their
        revision during the run, which follow the same schedule of blocks.
        The sieving tables are built once and shared. POSIX threads;
        \texttt{make THREADS=0} builds without them, and at most $33$
        threads sieve unless the library is built for more
        (Section~\ref{sec:build}). A search that cannot
        keep the points of a block fails with \texttt{RATPOINTS\_NO\_MEMORY}
        rather than deliver fewer points than it found. On a 16-core
        machine a search at height bound $4\cdot10^5$ runs $9.4$ times
        faster on 16 threads than on one and the long test suites $5.7$
        to $7.9$ times faster. Programs using the library must
        be recompiled, since \texttt{ratpoints\_args} has gained fields.
  \item \emph{The 512-bit build} has been run on AVX-512 hardware, where the
        test suite passes; see Section~\ref{sec:build} for when it makes sense
        to use it.
  \item \emph{Tests.} \texttt{make testthreads} runs the suites with several
        threads and with the blocks of the denominator loop fixed at other
        lengths, and \texttt{make tsan} runs them under ThreadSanitizer
        (Section~\ref{sec:tests}).
\end{itemize}

\textbf{Since version 3.1.0} (in the repository, to appear in the next
release): the compiled-in threshold \texttt{RATPOINTS\_SURVIVORS\_PER\_WORD}
is $0.002$ instead of $0.003$, which two faster machines found best and
which costs nothing on the author's laptop; on an Intel i7-13700K it makes
the random test curves $4.6\%$ faster at $16383$ and $2.2\%$ at
$2\cdot10^5$ and changes nothing on the point-rich ones
(Section~\ref{sec:retune}), and the ladder of \texttt{tune.sh} brackets the
new value. The manual now says that one thread per physical core beats one
per hardware thread (Section~\ref{sec:threads}).
...
\hline
\texttt{SURVIVORS\_PER\_WORD} & $0.002$ &
  the threshold at which the first stage stops taking moduli; fitted by
  \texttt{make tune} at $16383$, on the author's laptop and on two faster
  machines (Section~\ref{sec:retune}) &
  on the laptop $0.0015$ to $0.003$ cost nothing, $0.0045$ costs $2.7\%$
  and $0.0075$ costs $8.6\%$, the same at $2\cdot10^5$; on an i7-13700K
  and a Ryzen~9 9950X3D, $0.003$ costs $2$ to $3.6\%$ \\[3pt]
