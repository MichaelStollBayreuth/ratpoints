...
build for more cores is described in Section~\ref{sec:build}.
...
\textbf{Version 3.1.0} was released XXXX XX, 2026. The sieve can run on
several threads.
\begin{itemize}
  \item \emph{Threads.} The option \verb+-t+~$n$ and the field
        ...
        of a block are delivered when every earlier block has been: what
        ...
        to $7.9$ times, and on those suites two threads spend the
        processor cycles of one. Programs using the library must
  ...
  \item \emph{The 512-bit build} has been run on AVX-512 hardware, where the
        test suite passes; Section~\ref{sec:build} says what it is worth.
