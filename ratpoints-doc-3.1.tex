...
Threads pay for long runs. On the author's laptop, a search with a height
bound of $4\cdot10^5$ takes $1.9$~seconds on one thread and $1.0$, $0.7$
and $0.4$ on 2, 4 and~10; the sieve's work is divided without loss
(measured in processor cycles, two threads spend what one does), so what
the elapsed time shows beyond that is the machine -- a laptop's power
budget lowers the clock as more cores work. At a small height bound the
set-up of a search, which runs on one thread, is a large share of the run
and there is little to gain; many curves at a small height bound are
better searched by independent searches running in parallel
...
\begin{itemize}
  \item \emph{Threads.} The option \verb+-t+~$n$ and the field
        ...
        rather than deliver fewer points than it found. On the author's
        laptop a search at height bound $4\cdot10^5$ takes $1.9$~s on one
        thread and $0.4$~s on ten, and on the long test suites two threads
        spend the processor cycles of one. Programs using the library must
