...
such that the leading coefficient is a non-square mod~$p$. If~$p$ divides
the leading coefficient, it is the valuation of the denominator at~$p$ that
is restricted: the program works out which valuations can occur at all
(Section~\ref{sec:elim}), which typically rules out a power of~$p$, or
one particular valuation, or~$p$ itself, and each such prime counts as one
forbidden divisor. The program constructs a list of them against which to
check the denominators, and this option specifies how many of these should
be used. With \verb+-v+ it prints what it found.
...
It fires on 563 of the thousand random curves of \texttt{make test1}, whose
coefficients are small enough for 3, 5 and 7 to divide the leading one often,
and is worth $8.3\%$ there, $5.3\%$ of \texttt{make testdegrees} and
