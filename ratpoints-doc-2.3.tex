...
CCFLAGS1 = ${CCFLAGS256}
\end{verbatim}
in \texttt{Makefile}. It can be set to \verb+${CCFLAGS64}+ (word-size
registers, which works on essentially any machine), \verb+${CCFLAGS128}+,
...
> make test
\end{verbatim}
runs \texttt{test1}, \texttt{test1many}, \texttt{test2} and \texttt{timing}.
...
\begin{itemize}
  ...
  \item \texttt{testbase}, \texttt{testbase-many},
        ...
        \texttt{testbase2} --- the output they are expected to produce
...
constructs a list of them against which to check the denominators, and this
option specifies how many of these should be used. With \verb+-v+ it prints
what it found.
...
are inputs, and they come back from a call as they went in, with two
exceptions that are described below: the coefficient array and
\texttt{degree} when the polynomial has been reversed, and the flag bits
that report on the run. (Up to version 2.2.4 the program wrote some of
the values it chose or normalised into the input fields, so that a caller
who fills the structure once and then loops over curves had to reset them
before every call; this is no longer necessary.)
...
happened). Leading zero coefficients are dropped in the same way.
...
The program uses the array {\em domain} points to as its workspace when it
intersects the given intervals with the region where $f$ is positive
(unless \texttt{sturm} has a negative value, which may lead to a performance
loss); on return, \texttt{num\_inter} and the first \texttt{num\_inter}
entries of the array are what they were on entry.
...
only \texttt{cof} and \texttt{degree} (and whatever else changes from curve
to curve) need to be set; the other input fields keep their values across
calls, since version 2.3. \texttt{rptest.c} with the \verb+-O+ option does
exactly this, and \texttt{make test1once} checks that the points found are
the same as with the fields set once per curve.
...
list of denominators reduced modulo the sieving primes is stepped once per
...
The three constants above were tuned on my current laptop by minimising the
...
the run length at which a second-stage prime pays for setting itself up, over
both test sets, and writes the values it finds to a file
...
curves goes into building sieve tables, which neither constant has any effect
on, so a setting is judged partly on work it does not touch. \texttt{make
...
threshold or in the run length either way, or two more or fewer primes in the
second stage, is better. That is ten settings in a round rather than fifteen,
...
\texttt{E\_VALUES} and \texttt{U\_VALUES} give an absolute ladder, which is
what \texttt{make tune} uses, and \texttt{R\_FACTORS}, \texttt{E\_DELTAS} and
\texttt{U\_FACTORS} give a neighbourhood of the current settings --- multiples
of the threshold and of the run length, and offsets added to the middle
constant --- which is what \texttt{make tunehigh} uses and which takes
precedence when set.
...
Should you prefer to do it by hand, the two constants can be set directly:
\begin{verbatim}
> ./rptest -r 0.0075 -R 5 -z
> ./rptest-many -r 0.0075 -R 5 -z
\end{verbatim}
...
the \verb+-p+, \verb+-N+, \verb+-n+, \verb+-r+, \verb+-R+, \verb+-P+,
\verb+-Q+, \verb+-F+ and \verb+-S+ parameters, as well as \verb+-T+ (print the CPU time used, and
nothing else),
...
where $b'$ is the part of $b$ coprime to~$2l$.
...
share is half a per cent. \texttt{make tunehigh} tunes the two constants on
...
and its entry in the list of reduced denominators is stepped once per
...
two boundary words and the padding that makes the count a multiple of
\texttt{RATPOINTS\_CHUNK} are cleared after the phase instead of before it,
which comes to the same thing because \texttt{AND} is commutative. It is
...
back as they went in, and \texttt{sp1\_used}, \texttt{sp2\_used} and
\texttt{sp3\_used} report what was used; \texttt{make test1once} runs the
test curves that way. \texttt{make test3} runs the invocations of
\texttt{test3.sh}, one for each of these bugs, against \texttt{testbase3}.
