...
sieving loops by. \texttt{testhigh} uses the same thousand curves as
...
\begin{itemize}
  ...
  \item \texttt{testdata.h}, \texttt{testdata-many.h},
        \texttt{testdata-high-many.h} --- the curves used by the test runs
  \item \texttt{testbase}, \texttt{testbase-many},
        \texttt{testbase-high-many}, \texttt{testbase2} --- the output they
...
primes by quality alone, as before. The third, with $a = 0$, stops the
program from revising the number of primes while it runs
(Section~\ref{sec:adapt}); it revises them only when it chose them in the
first place, so \verb+-n+ or \verb+-N+ has the same effect.
...
\verb+-n+ and \verb+-N+ options. The fields
...
\texttt{sp3} $-$ \texttt{sp2} of them belong to the third. It is an output
field: what goes in is \texttt{sp3\_extra}, which fixes that difference when
...
compiled into \texttt{ratpoints.h} when negative.
...
tuning serve every height bound: the measured best offset is $3$ at a height
bound of $16383$ and $9$ at $2\cdot 10^5$, and using the smaller one at the
larger bound costs $8.6\%$ of \texttt{make testhighmany}. The first stage
needs no such correction, and measurement agrees that it does not: a
first-stage prime costs one \texttt{AND} per word unconditionally, so the
fixed part is a far smaller share of what it costs, and the best threshold
hardly moves with the height bound.
...
$n$~smallest~$r_p$, two such counts pin both terms; the rule for \texttt{sp2}
is then applied to the curve in hand rather than to the predicted one, and
the number of survivors a denominator brings to the third stage is a count
rather than an estimate. The first correction is made after a million
...
The two constants above were tuned on my current laptop by minimising the sum
of the times of \texttt{make test1} and \texttt{make test1many}. To repeat
that on your machine, run
\begin{verbatim}
> make tune
\end{verbatim}
which does the whole thing: it sweeps first the threshold and then the offset
over both test sets, and writes the values it finds to a file
...
threshold either way, or two more or fewer primes in the second stage, is
better. That is seven settings in a round rather than ten, and it rests on the
assumption that the two regimes do not want wildly different values. If it
moves a value it has not finished looking, and should be run again from there.
Expect an hour and a half at the default three rounds;
...
The candidates themselves can be chosen too. \texttt{R\_VALUES} and
\texttt{E\_VALUES} give an absolute ladder, which is
what \texttt{make tune} uses, and \texttt{R\_FACTORS} and
\texttt{E\_DELTAS} give a neighbourhood of the current settings --- multiples
of the threshold and offsets added to the other constant --- which is what
\texttt{make tunehigh} uses and which takes precedence when set.
...
tuning serve every height bound and is worth $8.6\%$ of \texttt{make
