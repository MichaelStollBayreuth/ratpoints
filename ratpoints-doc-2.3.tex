...
times more. At the height bounds the tuning runs use both factors are near
one and the threshold means what it says. At a height bound of $200$ the
first factor is a hundred and the second six: the stage stops after seven
moduli where it used to take thirteen, whose tables were a fifth of the run,
and the run is a quarter shorter; at $1000$ it is a seventh shorter, at $4000$
a per cent, and from $16383$ on nothing changes.
...
survivor reaches the extraction, which costs several times more. Both factors
are near one where the constant is fitted, so nothing was retuned or added
(section~\ref{sec:tuning} has the rule). At a height bound of $200$ the
stage takes $7$ moduli instead of $13$ --- a sweep of fixed counts puts the
optimum at~$7$ --- the tables fall from a fifth of the run to a twentieth, and
the run is $25\%$ shorter; $21\%$ at $100$, $15\%$ at $1000$ ($13\%$ on the
point-rich curves), $1\%$ at $4000$; at $16383$ and $2\cdot 10^5$ the counts
and the four suites do not move. What is left at $200$ is looking at the
thirty primes, a quarter of the instructions, and the exact checks.
