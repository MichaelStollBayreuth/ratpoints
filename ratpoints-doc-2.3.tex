...
survivors per word exceed the threshold times what the modulus costs per
word --- one \texttt{AND}, plus its tables and its per-denominator entries
spread over the $U$ words of the run --- with the survivors weighted by what
one costs downstream relative to a long run: there the second stage's eleven
...
$16383$ on the running time does not change.
