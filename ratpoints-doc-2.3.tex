...
\texttt{testbase4}; the whole takes about a second.  Before each invocation
...
and a sequence of searches on one initialised structure
...
The first stage has a correction of its own. A modulus is taken while the
survivors per word it removes --- the survivors it meets times $1-r$, $r$ its
density --- exceed the threshold times what the modulus costs per word ---
one \texttt{AND}, plus its tables, its per-denominator and per-call entries
and the fetch of its table row, spread over the $U$ words of the run --- with
the survivors weighted by what one costs downstream relative to a long run: there the second stage's eleven
moduli kill it cheaply, while in a run of a few thousand words there is no
second stage and every survivor reaches the extraction, which costs several
times more. At $2\cdot 10^5$ both factors are one; at $16383$, where
\texttt{make tune} fits the threshold, both are about three at the modulus
where the stage stops and nearly cancel, and the stage takes $0.4$ moduli
more than before. At a height bound of $200$ the cost factor is a hundred for
the small primes and several hundred at the stopping modulus, the downstream
factor six: the stage stops after seven moduli where it used to take
thirteen, whose tables were a fifth of the run, and the run is a quarter
shorter; at $1000$ it is a seventh shorter, at $4000$ a per cent, and from
$16383$ on the running time does not change. (Those figures are for the rule
with the survivors it meets; the factor $1-r$, the per-call cost and the row
fetch came later, with the estimate corrections of the tuning session, and
the threshold was refitted for the group.)
...
range of denominators --- thinned by the 2-adic pre-sieve, by the
...
looks at further primes whenever none in hand pays for itself and a prime
of the density this curve has been offering still would, not only when the
ones in hand run out (in its first form it stopped at the first prime that
did not pay, which on one curve of \texttt{testdata-high-many.h} left it
with none). When \verb+-P+ fixes the number of primes, the stage takes that
many without the test and looks further only when the ones in hand run out.
On those curves that turns one available prime into more than twenty.
...
The five constants above --- the threshold, the offset, the run length at
which a second-stage prime pays for setting itself up, what one row of a
sieving table costs, and what carrying a third-stage prime costs per
denominator --- were tuned on my current laptop by minimising the
...
the run length at which a second-stage prime pays for setting itself up, then
what one row of a sieving table costs, and last the third stage's cost per
denominator, over both test sets, and writes
...
\texttt{test1} and \texttt{test1many} use, two fifths of the time on the random
curves goes into work other than the two sieving phases --- choosing the
primes, the set-up per denominator, the reductions of the denominators, the
exact checks --- on which the threshold and the offset have little or no
effect (the sieve tables alone are $4\%$, and the cost of building a table is
the one constant that bears on them), so a setting is judged partly on work it
does not touch. \texttt{make tunehigh} measures on the large-height tests
instead, where the sieve is nearly
...
\begin{verbatim}
> ./rptest -r 0.003 -R 11 -U 1.6e6 -C 38 -Q 0.013 -z
> ./rptest-many -r 0.003 -R 11 -U 1.6e6 -C 38 -Q 0.013 -z
\end{verbatim}
...
to achieve a comparable reduction of point tests. Since version~2.3 the
candidates are ranked by what they say per unit of what they cost rather
than by the fraction alone, and they are not all primes: the prime powers
and the products of small primes up to~$64$ take part, with the product of
their factors' fractions, and a candidate sharing a prime with one already
taken is passed over; see the changes of that version below.
...
version~2.3; the modulus is discussed with the other changes of that
version below). Then for every
...
are that much shorter. Before version~2.3 only the
restriction to one parity was made; see the changes of that version below.
...
program now looks at further primes when that happens (and when the third
stage asks for them, see below); the
...
for two fifths of the run to be set-up and overhead rather than the two
sieving phases (the sieve tables alone were a fifth of it when the suites were
added, and are $4\%$ after the work on the set-up), so it measures a change to
the sieving loops through a filter; at the new one the two phases are nine
tenths of the run and the tables a third of a per cent. \texttt{make
tunehigh} tunes the constants on them. The test programs also no longer pass
...
numerators modulo~64, and that pattern used to be written into all of them on
...
a seventh of all the bit arrays swept in \texttt{make test1}, two fifths of
...
left, up to fourteen rows past them, which is what the wrap-around copies at
...
The information from the polynomial modulo a power of~$2$ is taken
modulo~$64$ instead of modulo~$16$. The pattern of admissible numerators
that the first prime \texttt{AND}s into every bit array is one repeated
$64$-bit word whatever its period, so the finer modulus costs the sieve
nothing, and the squares are $12$ of the $64$ residues modulo~$64$ against
$4$ of~$16$: where $b^D f(a/b) \equiv 0 \bmod 4$ the coarser modulus let
twice as much through. The even denominators are decided exactly as well:
with $a$ odd, $F(a,b) = a^D\, \tilde f(b/a)$, where $\tilde f(t) = t^D
f(1/t)$ is the reversed polynomial, an integer polynomial in~$t$, and $a^D$
is the square of a unit, so one table of $\tilde f$ modulo~$64$ replaces
the hand-derived congruences. On random curves the first phase needs half
a prime less and performs $4.5\%$ fewer \texttt{AND}s; the set-up costs
$2260$ instructions more per curve, so a run at height~$100$ is $0.7\%$
longer and the gain begins at height~$200$. It is worth $3\%$ of
\texttt{make test1} and of \texttt{make testhigh}, $1.5\%$ of \texttt{make
test1many}, about $1\%$ of \texttt{make testhighmany}, and between $2$
and $3\%$ at height bounds from $200$ to~$4000$. Modulo~$256$ would remove
one admissible class in twelve more, a quarter of what the step from $16$
to~$64$ removes, for sixteen times the set-up, and is not done.

The choice of primes for the third sieving stage no longer starves. The
stage used to take primes from those already looked at while each paid for
itself and stop at the first that did not, asking for more only when none
were left; on a curve with very many rational points the small primes say
nothing and the informative ones come late, so one curve of \texttt{make
testhighmany} was left with no third stage at all and $250\,000$ exact
checks in place of $7900$. Now the stage looks at a further prime whenever
none in hand pays but a prime of the density the curve has been offering
would; the same test decides whether an empty pool is refilled at all, and
a number of primes fixed with \verb+-P+ is taken as before. And the estimate of the run's length counts the numerators of even
denominators at half width, which was what the sieve then swept for them: it was
too large by up to a third on such curves, and its spread over the random
curves halves. Worth $1\%$ of \texttt{make test1many}, half a per cent of
\texttt{make testhighmany} and nothing measurable elsewhere; the exact
checks of \texttt{make testhighmany} fall by a factor of~$4.6$.

The bit arrays hold only the numerators a denominator can have modulo a
power of two. For each class of the denominator mod~$64$ the admissible
numerators mod~$64$ are known (the paragraph before last); on two fifths of
the random test curves those of the odd denominators lie in a single class
modulo $4$ or~$8$, and those of the even denominators often do as well. The
program used to notice only whether they were all odd or all even, and pack
every second numerator; now every class gets the largest stride $2^k$ such
that all its admissible numerators are congruent modulo~$2^k$, and a bit of
its arrays stands for every $2^k$-th integer, $a_0 + 2^k t$. The sieve
tables serve unchanged: bit~$t$ wants the residue $(a_0 + 2^k t)\, b^{-1} =
(t + a_0 2^{-k})\, (b\, 2^{-k})^{-1} \bmod p$, so the row is the one for
$b\, 2^{-k} \bmod p$, read $a_0 (2^k R)^{-1} \bmod p$ bit arrays further on,
with $R$ the number of bits in a bit array --- which is what the two-fold
packing already did with $k = 1$ --- and the sieving loops do not change.
On the random curves the first phase sweeps $15\%$ fewer bit arrays and
performs $12\%$ fewer \texttt{AND}s (the rule then takes three quarters of
a prime more into the first phase), and the work done once per denominator
got simpler. Worth $9\%$ of \texttt{make test1} ($7.4$ to $8.8\%$ at three
placements of the code), $12.5\%$ of \texttt{make testhigh}, $1\%$ of
\texttt{make test1many}, $1.5\%$ of \texttt{make testhighmany} and $3\%$ at
the height bound~$4000$. A run at height~$1000$ is $3.5\%$ longer and one
at~$200$ $5\%$ longer: deciding the $64$ classes costs about $7000$
instructions per curve, and the extra first-phase prime brings tables that
a run of that length cannot use --- the rule that counts the first-phase
primes knew nothing of their tables, which predated this change and is put
right two paragraphs below.

The sieve can use composite moduli. A table row only has to be periodic in
the bit index with the modulus as its period and be selected by the
denominator's residue, and nothing in the sieving loops asks for a prime;
the program used primes only, and the small ones say little for what they
cost: modulo~$3$ two residues in three are admissible on a random curve, so
that \texttt{AND} removes a third of the candidates where a large prime's
removes half. Modulo~$9$ the admissible residues are those with $f(x)$ a
square mod~$9$, and the squares are $4$ of the $9$ residues: over the
thousand random test curves that carries twice what $3$ does, for a table
of nine rows, and $25$, $27$ and $49$ do the like for $5$, $3$ and~$7$; for
$p \mid b$ the row is built from the reversed polynomial, as the mod-$64$
one is. The row of a product of coprime moduli is the \texttt{AND} of its
factors' rows, so one \texttt{AND} per word carries them all, and by the
Chinese remainder theorem its density is the product of theirs: $45 = 9
\cdot 5$ carries what two large primes do. Every odd composite up to~$64$
is now a candidate in the ranking that chooses the primes, with that
density and its own table cost, and moduli sharing a prime exclude one
another; the third stage keeps to primes, and below a height bound of a
few hundred no table pays and nothing is offered (at~$1000$ the modulus~$9$
enters on a third of the random curves, and the run does not change
measurably). The first phase of a random curve at height~$16383$ uses $10$
moduli instead of $11.7$ primes and performs $14\%$ fewer \texttt{AND}s.
Worth $7\%$ of \texttt{make test1} ($6$ to $8\%$ in two measurements),
$11\%$ of \texttt{make testhigh} and $3$ to $4\%$ of \texttt{make
testhighmany}, and nothing measurable on \texttt{make test1many}, where the
small moduli are nearly silent. Larger moduli save more instructions and
lose time: a row
of $220$ bit arrays does not stay in the first-level cache, and the cost
model prices an \texttt{AND} the same whatever the modulus; the bound is
\texttt{RATPOINTS\_COMPOSITE\_MAX} in \texttt{ratpoints.h}. The measurement
behind that bound was made afterwards, one curve at a time: an \texttt{AND}
costs one cycle while the first stage's rows together fit in some $25$~KB and
about $0.8\%$ more per further KB; the automatic count sits at the optimum a
sweep of fixed counts finds on every suite and height, so the bound is what
keeps the row set small on the point-rich curves, whose informative primes
are large, and the larger products pay only on random curves at
$2\cdot 10^5$ ($3.7\%$ of \texttt{testhigh}) while costing $4.5\%$ of
\texttt{testhighmany}. Telling the two apart would take a model of the row
set with several cache constants; the bound stays.

The rule that ends the first stage weighs what a modulus costs and what its
survivors cost. It used to take moduli while the expected survivors per word
exceeded a fitted constant, which stands for the ratio of two costs --- one
more \texttt{AND} per word against what a surviving word costs from there on
--- both frozen at the values of the height bound the constant was fitted at.
Now the modulus's side carries its tables and its per-denominator entries
spread over the words of the run, the same per-word cost the ranking already
charges, and the survivor's side is weighted by what one costs with the
second stage the run will have: in a long run eleven cheap \texttt{AND}s
kill it, in a run of a few thousand words there is no second stage and every
survivor reaches the extraction, which costs several times more. The two
factors nearly cancel at $16383$, where the constant is fitted, and are one
at $2\cdot 10^5$, so nothing was retuned or added (section~\ref{sec:tuning}
has the rule). At a height bound of $200$ the stage takes $7$ moduli instead
of $13$ --- a sweep of fixed counts puts the optimum at~$7$ --- the tables
fall from a fifth of the run to a twentieth, and the run is $24\%$ shorter;
$20\%$ at $100$, $14\%$ at $1000$ ($13\%$ on the point-rich curves), $1\%$
at $4000$; at $16383$ the stage takes $0.4$ moduli more and the two suites
are within the noise, at $2\cdot 10^5$ nothing moves. What is left at $200$
is looking at the thirty primes, a quarter of the instructions, and the
exact checks. (Those figures are for the rule with the survivors a modulus
meets; the factor $1-r$ for the ones it removes, a cost per call of the sieve
and the fetch of its table row came later, with the estimate corrections of
the tuning session, and the threshold was refitted for the group.)

The estimates the rules run on were corrected as one group at the end, and
the constants refitted with them, because each correction alone looks worse:
the constants had been fitted with the old estimates and absorbed their
errors. The run shape counts the words the sieve sweeps, each interval of
each denominator rounded outwards to whole bit arrays with the ends the
height bound cuts placed exactly (it had counted numerators, a third too few
at height $1000$ and a tenth at $4000$); the Jacobi test keeps $0.53$ of the
denominators, not half; the bits set per word are averaged over the classes a
run visits, weighted by the words each sweeps; a prime's density counts the
class of denominators it divides at what its row admits; the rule that ends
the first stage weighs the survivors a modulus removes, not the ones it
meets; and a first-stage modulus is charged for each call of the sieve and
for fetching its table row once per denominator, two costs the model had none
of and the run at height $1000$ pays five times over for every \texttt{AND}.
With the estimates right to a few per cent at every height, the threshold
\texttt{make tune} fits at $16383$ moved from $0.0075$ to $0.003$ and holds
at $2\cdot 10^5$ and at $200$ to $4000$ alike, where before the corrections
the group wanted a different threshold at every height. The suites are where
they were, within $2\%$ either way; what changed is under the hood: at a
fixed count of ten moduli the corrected ranking runs the point-rich curves at
height $1000$ in $0.62$ of the time, since the old table term, spread over a
third too few words, had kept nearly silent small primes in the first stage
there.

What all of it comes to, against the released 2.2.4 on the same curves and
the same laptop (cycles pinned to one core, medians of three interleaved
rounds; 2.2.4 at its own defaults, which sieve with the primes below $128$
where this version uses those below $256$; both find the same points on every
suite):
\begin{center}
\begin{tabular}{lrrrr}
...
bound by its branches (the Jacobi symbols, the test for common factors, the
padding to whole chunks), and cycles fall two to three times more than
instructions there.
...
modulo~16 (the modulus of the time) admitted any numerator, the array of numerator patterns was read
...
machines, being the ratio of two different kinds of work. The tuning session
at the end of 2.3 added a fifth stage for the third stage's cost per
denominator (\verb+-Q+), which had been fitted when that stage was set up
for every denominator and is set up on demand since.
