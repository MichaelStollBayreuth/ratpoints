...
primes, the set-up per denominator (the sieve tables alone are $4\%$; their
cost is the one constant that bears on it), the reductions of the
denominators, the exact checks --- on which the threshold and the offset have
little or no effect, so a setting is judged partly on work it does not touch. \texttt{make
...
tunehigh} tunes the constants on
