...
By default, the program has the odd primes below~$256$ available and
...
If you intend to do computations with large height bounds, it may make sense
to make more primes available. This can be achieved via
...
otherwise it is taken to be~8. The precise meaning is that the program will
...
(they were $22\%$ when these suites were added, before the set-up was
reworked) but $0.3\%$ of \texttt{make testhigh}, the whole of the set-up per
denominator $8.5\%$ against $1.3\%$, and the two sieving phases themselves go
...
one or more for each of the bugs fixed in versions 2.2.4 and~3.0.0, and
compares the output against \texttt{testbase3}.

\begin{verbatim}
> make test4
\end{verbatim}
runs the invocations of \texttt{ratpoints} collected in \texttt{test4.sh},
a few hundred of them, chosen so that every branch of the code runs at
least once: degrees 1 to 9 and up to the maximum of 100, odd and even, square and non-square
leading and constant coefficients, every reason to reverse the polynomial,
curves with no points modulo some prime or no admissible numerator modulo
64, forbidden divisors of every kind, every option, restricted denominator
...
phase; \texttt{USE\_LONG\_IN\_PHASE\_2}, the second phase one word at a
time; \texttt{PRIME\_SIZE} 7, the smaller prime table; and composite
moduli off and unbounded.  Each is
...
same \texttt{testbase4}; the build with the smaller table is compared
without the reports of \verb+-v+, two of which say what the primes beyond
127 say.  About a minute.  The script exits with status 1 when an output
...
builds the sources instrumented and unoptimised in
\texttt{build-coverage/}, runs the tests through that build (all of
\texttt{make test} but \texttt{test2} and \texttt{timing}, which add
nothing but minutes) and prints, for each source file, the lines executed
and the branches taken at least once (\texttt{coverage.sh}); the
annotated sources are left in that directory.
When this was written the tests executed $99.9\%$ of the lines of
\texttt{find\_points.c}, $96\%$ of \texttt{sift.c}, all of \texttt{init.c}
...
\texttt{testapi} and \texttt{timing}, each of them whatever the earlier
ones did, and fails at the end if any of them failed, so that a script can
tell whether the build passed.
...
\begin{itemize}
  ...
  \item \texttt{ratpoints-doc-3.0.pdf} --- this documentation file
...
and the specification of the search intervals, which must be given in order.
...
\verb+-R+ a flat offset again, as it was before version~3.0.0. The second sets
...
before~3.0.0 did in effect, namely assume that every curve costs what a
...
keep their values across calls, since version~3.0.0. \texttt{rptest.c} with
...
since version~3.0.0), and those that pass are then checked exactly.
...
candidates are ranked by what they say per unit of what they cost rather
than by the fraction alone (Section~\ref{sec:spchoice}), and they are not
all primes: the odd prime powers and the products of small primes up to~$64$
take part, with the product of their factors' fractions --- a table row only
has to be periodic in the bit index with the modulus as its period, and the
row of a product of coprime moduli is the {\em and} of its factors' rows ---
and a candidate sharing a prime with one already taken is passed over.
...
extended. We first find all solutions mod~$64$ (mod~$16$ before
version~3.0.0). Then for every
...
in one residue class modulo a power of two --- they are all odd for an even
denominator, and on many curves those of the odd denominators lie in one
class modulo $4$ or~$8$ --- we can restrict the sieving to such numerators:
a bit of the bit arrays then stands for every $2^k$-th integer, for the
largest such~$k$ ($2$, $4$ and $8$ occur on the test curves), and the arrays
are that much shorter. Before version~3.0.0 only the
restriction to one parity was made.
...
where $b'$ is the part of $b$ coprime to~$2l$. Since version~3.0.0 that symbol
...
values are compiled into \texttt{ratpoints.h}, \texttt{make tune} measures
...
prime~$p$ --- and for each odd prime power and product of small primes
up to~$64$, which sieve in the same way (Section~\ref{impl}) --- the
density~$r_p$, the proportion of residues~$a$ modulo~$p$ for which $f(a)$
is a square. The expected proportion of numerators surviving a set of
...
(the numerators of one denominator are a piecewise linear function of it, so
a sample over the denominators is accurate to a fraction of a per cent), and
ranks the moduli by
...
bit per word whatever the curve: at about seven moduli for a run of a few
hundred numerators per denominator, at about ten at the height bound
$16383$ of the test suites. The second stage then gets
\[ \texttt{sp2} = \texttt{sp1}
   + \frac{\texttt{RATPOINTS\_SP2\_EXTRA}}{1 + \texttt{RATPOINTS\_SP2\_U0}/U} \]
further primes, capped by the number that carry any information at all.
...
second stage keeps to primes and has a ranking of its own, since a prime
there is applied only to the bit arrays that survived the first stage, and
its table therefore weighs some three times more heavily against it.

A curve with very many rational points makes $f$ a square modulo every
residue for the smallest primes. Those primes say nothing and are discarded,
...
primes, so this stops of its own accord, and no random curve ever triggers
it. Giving \verb+-p+ explicitly switches it off: an explicit number of
primes is a hard limit, as are \verb+-n+ and \verb+-N+ for the two stages.

\subsection{How many primes the third stage uses}\label{sec:sp3choice}

The third stage tests the survivors of the first two one at a time, against
...
random curve at a small height bound the stage uses no prime at all; on a
curve with many rational points at a large one it uses twenty or more, and
...
that last because the test for common factors runs before this stage, and
has to: a numerator sharing a factor with the denominator stands for a
fraction a smaller denominator has dealt with already, so $f$ takes the same
value there and every prime accepts it.
...
change; \verb+-W+~$w$ overrides the estimate with $w$ cycles, and \verb+-v+
...
construction: if $a/b$ is a rational point, so is $ka/kb$ for every~$k$ ---
it is the same rational number, with the same value of~$f$ --- and it passes
...
see the floor because it is sieving: it counts the bit arrays that survive
the first stage, the survivors of the second and the candidates that pass
the test for common factors, all of them on paths that are taken rarely, and
fits
...
\texttt{sp2} as well, and \verb+-A 0+ switches the revision off. Changing
the number of primes part-way through a run is safe: sieving only ever
removes numerators that cannot be points, so what is found does not depend
on it. Nothing that was fixed with \verb+-n+, \verb+-N+ or \verb+-P+ is
revised.
...
\texttt{U\_VALUES} and \texttt{C\_VALUES} give the ladders of candidates.

\begin{verbatim}
> make tunehigh
\end{verbatim}
measures on \texttt{make testhigh} and \texttt{make testhighmany} instead,
at a height bound of $2\cdot10^5$, where the two sieving phases are nine
tenths of the run; at the $16383$ of \texttt{test1} two fifths of the time on
the random curves goes into work the constants have little or no effect on.
...
length or in the table cost, or two more or fewer second-stage primes, is
better (\texttt{R\_FACTORS}, \texttt{E\_DELTAS}, \texttt{U\_FACTORS} and
\texttt{C\_FACTORS} set that neighbourhood). Expect a couple of hours at the
...
\textbf{Version 3.0.0} was released September 19, 2026. Its theme is that
...
\begin{itemize}
  ...
  \item \emph{A third sieving stage} tests the survivors of the two
        ...
        option~\verb+-A+).
  \item \emph{Composite moduli.} Odd prime powers and products of small
        primes up to~$64$ sieve alongside the primes: modulo~$9$ carries
        twice what $3$ does, and one \texttt{AND} with the row of
        $45 = 9\cdot5$ carries what two large primes do
        (\texttt{RATPOINTS\_COMPOSITE\_MAX} in \texttt{ratpoints.h}).
  ...
  \item \emph{Denominators.} The test at a prime dividing the leading
        coefficient, derived from the Newton polygon of~$F$, runs for the
        first time --- it had sat behind a guard it could never pass, and was
        wrong besides (Section~\ref{sec:elim}); the forbidden-divisor bit
  ...
  \item \emph{Library.} \texttt{ratpoints\_args} has gained fields, so
        ...
        come back as they went in, except \texttt{num\_inter} and
        \texttt{domain}, which describe the region searched, and
  ...
  \item \emph{Tests.} \texttt{make test} runs, besides the random curves,
        ...
        bug fixed (\texttt{test3}), a suite that exercises every branch of
  ...
  \item \emph{Bugs fixed.} All of those listed under version~2.2.4, and in
        addition: the estimate of the run length fell to its floor when no
        odd denominator admitted a numerator, which switched the second and
        third stages off for such curves; at a height bound of~$2^{62}$ the
        third stage's bias overflowed and rejected every point; an interval
        endpoint of the positivity region was scaled by a shift that is
        undefined at large bisection depths (now \texttt{ldexp}); and the
        library refuses a negative degree.
\end{itemize}
...
Version 3.0.0 was developed in a series of experiments, each recorded in a
notebook on its own branch of the repository. This appendix keeps what a
reader would need in order to change the program without repeating them:
how the measurements were made and what they can and cannot resolve, where
the time goes, why curves with many rational points behave so differently
from random ones, the cost model with the origin of each constant, what
each change was worth, what was tried and did not pay, and how much of all
this depends on the machine. Every figure was taken on one laptop --- an
Intel i7-1355U, the code pinned to one performance core, the 256-bit build,
gcc~14 --- and is quoted as a ratio of cycle counts against the version
before the change; Section~\ref{app:machine} says what would come out
...
better evidence; but it is blind to a data-layout cost, and one change
measured that way gave back more than half of a $2.8\%$ gain across builds
because it had grown a structure read in the innermost loop from 40 to
48~bytes. Both methods were used when a change touched such a structure.
...
all. With every change of this version in place, on the random test
curves:
\begin{center}\small
\begin{tabular}{lrr}
...
sieve, when this was measured midway through the work, the first stage was
three fifths of a long run on random curves and three quarters on
point-rich ones, the second stage with the exact checks a quarter and a
fifth. The exact check itself is under one per cent of every suite --- it
was never a target --- and the integer divider is busy for less than $0.6\%$
of the cycles, so the divisions were never what cost anything either: the
two to three per cent that replacing the division-avoiding subtraction
chains by a multiplication bought came from twenty fewer instructions per
call. At a height bound of~$200$ the shape is different again: examining
the thirty primes is a quarter of the instructions, and the tables were a
fifth of the run before the first-stage rule learnt to price them and are a
twentieth since.
...
curves behave very differently in it. On a random curve about one
numerator in $10^4$ survives, and the rule of Section~\ref{sec:spchoice}
reaches its target with ten or twelve moduli. On a curve with very many
rational points $f$ is a square modulo every residue for the smallest
primes, so those primes carry no information and are discarded, the rule
needs twenty or more primes, and with the thirty primes below $128$ that the
program used to look at about one such curve in five could not reach its
target at all; that is why the table holds the primes below~$256$ now and
the program looks further when it needs to, which is worth a factor of
$1.3$ to~$2$ on those curves at a large height bound and costs nothing on
any other. Larger primes are what these curves need: the proportion of
admissible residues tends to one half as the prime grows however many
points the curve has, while for the small primes it is close to one.
...
test runs before the third stage. The floor is what made \texttt{sp2}
unpredictable from the densities alone (Section~\ref{sec:adapt}), and it is
why the number of survivors a denominator brings to the third stage ranges
over a factor of $260$ between the suites --- $0.035$ on the random curves at
$16383$, $9.2$ on the point-rich ones at $2\cdot10^5$ --- so that no fixed
number of third-stage primes can serve and the rule there is a stopping
rule.

The two regimes want the same constants, which was not obvious and was
measured twice: \texttt{make tunehigh} at three rounds keeps every value
that \texttt{make tune} finds at $16383$, all within $0.7\%$ except the
threshold, which costs $5.6\%$ when doubled there and $8.6\%$ at two and a
half times its value here. One threshold also serves every height bound
from $200$ to $2\cdot10^5$ since the estimates behind the rules were
corrected (Section~\ref{app:model}); before that the group wanted a
different threshold at every height.
...
counting the bits set on entry to the first stage was $18\%$ of an
instrumented run of \texttt{make testhigh} until it became a multiplication
--- so shares are taken from a timing-only build. Table~\ref{tab:constants}
lists the constants.
\begin{table}[tbp]\footnotesize\setlength{\tabcolsep}{4pt}
\begin{tabular}{p{3.3cm}p{1.5cm}p{4.6cm}p{4.9cm}}
constant (\texttt{RATPOINTS\_\dots}) & value & what it is, and how it was found & how flat \\
\hline
\texttt{SURVIVORS\_PER\_WORD} & $0.003$ & the threshold at which the first stage stops; fitted by \texttt{make tune} at $16383$ & $0.0045$ costs $2.7\%$, $0.0075$ (the value before the corrected estimates) $8.6\%$, $0.0015$ to $0.002$ nothing; the same at $2\cdot10^5$ \\
\texttt{SP2\_EXTRA}, \texttt{SP2\_U0} & $11$, $1.6\cdot10^6$ & the second-stage offset of an arbitrarily long run, and the run length at which a prime's set-up costs as much as its sieving; fitted to four measured optima --- 3, 5, 9 and 12 second-stage primes at $U = 5.9\cdot10^5$, $5.1\cdot10^6$, $8.2\cdot10^7$ and $7.1\cdot10^8$ words --- which the pair reproduces as 3, 8, 11, 11 & a flat offset costs about $5\%$ at one height bound or the other, and the old flat~$5$ costs $12\%$ of \texttt{testhighmany}; only \texttt{tune} and \texttt{tunehigh} together separate the two \\
\texttt{COST\_TABLE} & $38$ & one row of a sieving table; measured at 18 to 30 & flat from 20 to 70: $0.9515$ at 38 against $1$ at 0, $0.9619$ at 140 \\
\texttt{COST\_BP}, \texttt{COST\_SETUP} & $8$, $30$ & a denominator's entry in the list of reduced denominators, and the fill of a sieve entry per denominator; measured & 8 against 24 within $0.3\%$ \\
\texttt{COST\_CALL}, \texttt{COST\_LINE} & $50$, $4.5$ & a call of the sieve per modulus (12 to 13 cycles) and the fetch of a table row per denominator (1.2 cycles per cache line); measured for the corrected estimates & not swept singly; part of the group whose refit moved the threshold \\
\texttt{COST\_PHASE2}, \texttt{COST\_SURVIVOR} & $110$, $1400$ & one second-stage \texttt{AND}, and one survivor of the second stage; the first measured at 155 (random) and 61 (point-rich) at $2\cdot10^5$ & 60, 100 and 220 within about $2\%$ of 110 \\
\texttt{SP3\_PER\_SURVIVOR}, \texttt{SP3\_PER\_DENOM} & $0.055$, $0.013$ & the third stage's costs per survivor tested and per denominator, as fractions of one exact check (250 to 280 cycles beyond the first for a denominator, some 190 once per denominator, 3.5 per step carrying $b$ past a prime) & the per-denominator value from $0.003$ to $0.05$ within $1.2\%$; a fixed \verb+-P+ never beats the rule by more than $0.3\%$ \\
\texttt{CHECK\_STEP}, \texttt{\_LIMB}, \texttt{\_CALL}, \texttt{\_ROOT}; \texttt{CHECK\_REFERENCE} & $29$, $8$, $94$, $170$; $306$ & the exact check: a Horner step, a further limb, the call, a further limb of the root, in cycles, and the value for a degree-6 curve with small coefficients; from \texttt{make bench\_check} (in the sieve the check costs $44 + 1.44$ times the microbenchmark) & only ratios between curves enter; the estimate agrees with the sieve to $6\%$ over degrees 3 to 12; scaling every curve's value alike from 306 to 1000 moves no suite by $0.5\%$ \\
\end{tabular}
\caption{The constants of the cost model: their values, where they came from, and how much a different value costs.}\label{tab:constants}
\end{table}
...
is why the per-curve estimate of the exact check's cost changed no running
...
of a modulus --- had to be corrected as one group and the constants refitted
with them, because each correction alone looked worse: the constants had
been fitted with the old estimates and absorbed their errors. After the
corrections the predicted words per denominator are within $1.5\%$ of the
count at every height bound from $200$ to $2\cdot10^5$, and the automatic
choice of the first-stage moduli beats or ties the best fixed \verb+-n+ in
every one of eighteen combinations of suite, height bound and modulus cap.
A finer point of the same kind: three conventions for the density of a
prime and of its powers were measured, and the mixed one --- the prime
approximated, the power exact --- was best on every suite. A fitted model
absorbs its approximations, and ``more exact'' can be worse.

\subsection{What each change was worth}\label{app:worth}

Cycles after the change over cycles before it, on the four suites (random
and point-rich curves at $16383$ and at $2\cdot10^5$) and, where it matters,
at the small height bounds; each measured as described above against the
program as it stood when the change was made, so the rows do not multiply
out to the totals of Section~\ref{sec:changelog}. A dash is a change
within the noise. Table~\ref{tab:worth} has them.
\begin{table}[tbp]\footnotesize\setlength{\tabcolsep}{4pt}
\begin{tabular}{p{5.9cm}rrrrp{3.1cm}}
change & \texttt{test1} & \texttt{many} & \texttt{high} & \texttt{highmany} & elsewhere \\
\hline
\texttt{sp1}, \texttt{sp2} from the survival rate; the tight scan over the empty arrays (against 2.2.3) & $0.86$ & $0.64$ & & & \\
further primes for starved curves, \texttt{PRIME\_SIZE}~8 & -- & -- & -- & -- & $0.50$ to $0.75$ on the curves it applies to \\
the survivors found by counting trailing zeros & $0.99$ & $0.95$ & $0.98$ & $0.98$ & \\
the table set-up: the branch on the residue test becomes a shift, four rows at a time & $0.87$ & $0.98$ & -- & -- & the tables $3.7\times$ faster \\
the third sieving stage & -- & $0.98$ & $0.98$ & $0.93$ & \\
reductions by a multiply-high instead of a subtraction chain & $0.97$ & $0.97$ & $0.98$ & $0.98$ & \\
the run length known in advance: cost-aware ranking, the offset scaled, the survivors counted, the check's cost estimated & $0.94$ & -- & -- & $0.88$ & degrees $0.86$ \\
the first stage writes the arrays: no fill pass & $0.97$ & -- & $0.94$ & $0.97$ & degrees at $2\cdot10^5$ $0.95$ \\
the valuation test at primes dividing the leading coefficient & $0.92$ & $0.98$ & $0.91$ & $0.98$ & degrees $0.95$ \\
the scan into a sentinel; the second stage's row by the reciprocal & $0.97$ & $0.97$ & $0.93$--$0.96$ & $0.93$--$0.96$ & \\
the denominator side: Legendre-symbol tables, 64 denominators a word, reciprocals, a branchless gcd, forbidden divisors to $\sqrt H$, the third stage set up on demand & $0.76$ & $0.96$ & $0.94$ & $0.98$ & \\
no padding: the tail in legs of 8, 4, 2, 1 & $0.90$ & $0.94$ & $0.98$ & $0.98$--$1.00$ & $0.81$ at 1000 and 4000, $0.92$ at 200 \\
the 2-adic information modulo 64 & $0.97$ & $0.98$ & $0.97$ & $0.99$ & $0.97$--$0.98$ at 200 to 4000 \\
the third stage looks further; even denominators at half width & -- & $0.99$ & -- & -- & the exact checks of \texttt{highmany} $4.6\times$ fewer \\
each denominator class packed with its 2-adic stride & $0.91$ & $0.99$ & $0.875$ & $0.98$ & $0.97$ at 4000, $1.035$ at 1000, $1.05$ at 200 \\
composite moduli up to 64 & $0.92$ & -- & $0.89$ & $0.96$ & nothing below a few hundred \\
the first-stage rule prices the tables and the downstream cost & -- & -- & -- & -- & $0.76$ at 200, $0.86$ at 1000, $0.99$ at 4000 \\
the estimates corrected as a group, the constants refitted & $0.995$ & $0.985$ & $0.993$ & $0.981$ & $1.00$ to $1.03$ at 200 to 4000 \\
\end{tabular}
\caption{What each change was worth: cycles after over cycles before, on the random and the point-rich curves at $16383$ (\texttt{test1}, \texttt{many}) and at $2\cdot10^5$ (\texttt{high}, \texttt{highmany}).}\label{tab:worth}
\end{table}
The suite of curves of degrees 3, 4, 7 and~8 gained most from the changes
to the model because it is the one set none of the constants had been
fitted on: rules carry over where fitted constants do not. The largest
single gain on the random curves at $16383$ came from the denominator side,
where the Jacobi symbol computed by a binary algorithm for every
denominator had been $12\%$ of the run and a third of its mispredicted
branches; the largest at $2\cdot10^5$ from the stride, which sweeps $15\%$
fewer bit arrays.
...
\begin{itemize}
  \item \emph{The second stage on narrower registers.} The guess was that a
        ...
        (what \texttt{USE\_LONG\_IN\_PHASE\_2} now means) is a wash to $4\%$
        slower: a narrow and a wide read of the same table come from one
        cache line, so there was never any traffic to save.
  ...
  \item \emph{Sieving tables without the replication.} The tables hold each
        ...
        in eight split a line. Ordering the tables by prime was already the
        case in effect, since they are built lazily in the order of use, and
        the TLB is not a constraint.
  \item \emph{Testing several bit arrays at once in the scan} paid when the
        number of first-stage primes was fixed and the proportion of
        surviving arrays varied widely; with the rate pinned at a few per
        cent a group containing a survivor wastes its whole \texttt{OR}, and
        a group of one wins. Per-chunk survivor flags computed in the first
        stage were prototyped three ways: the branchy form mispredicted, the
        branchless forms spilled, since all sixteen vector registers hold
        live arrays. The sentinel was the answer.
  ...
  \item \emph{Blocking the prime loop for the cache} and \emph{a two-copy
        ...
        performance cores they lose, on its efficiency cores the review
        predicted gains of $9\%$ and of up to $46\%$ of the first stage. Their
        sign depends on the cache size, so they belong behind a run-time
        gate and a measurement on such a machine, and were not built.
\end{itemize}
...
\begin{itemize}
  \item Bit $j$ of \texttt{den\_bits} is the class $b \equiv j \bmod 64$, not
        ...
        the loop shifts before it tests. Read the other way it inverted the
        parity for a third of the curves and made the run-length estimate
        $2.4$ times too small, and it looked like a modelling error.
  \item Changing the number of moduli in the middle of a run is safe for
        ...
        suddenly build a table. The last was a segmentation fault.
  \item Anything added to the sieve entry structure is read in the innermost
        loop of the first stage; eight more bytes gave back half of a gain.
  \item The sixteen-register ladder of the first stage stays hand-written:
        ...
        of~\texttt{0} kept eight index registers live and spilled five per
        prime.
