...
incarnations: Opus~5, Fable~5.1) to optimize the code and find and fix
...
the final program run 2 to~10 times faster now (depending on the kind
of curve and the height bound). Based on this aggregate very noticeable
...
The next version, v3.1.0, will include multi-threading abilities.
Stay tuned!
...
on these up to a hundred times more, % @CLAUDE: is this still true?
so the best division of labour between
...
(which check that the bugs fixed in versions 2.2.4 and~3.0.0 stay fixed) and
...
chosen so that every branch of the code runs at
least once: degrees $1$ to~$9$ and up to the maximum of~$100$,
...
Three further options control the same choice, allowing to switch off
or measure certain aspects of the prime-selecting stage: \\
...
a height bound of~$16\,383$ the stage does nothing at all, % @CLAUDE: check!
...
points. It effectively sets the \verb+-y+ option, since the
$y$-coordinates are not computed. This implies that the $x$-coordinates
that are output may not actually correspond to rational points.
...
(option~\verb+-F+). The field~\texttt{array\_size} specifies the maximal size
...
candidates include the odd numbers below~$64$
...
root of the height bound (but at most up to~$1021$), % @CLAUDE: check!
building the patterns for the
...
could. However, the subtraction chains written to avoid divisions turned
out to not be that fast either, mostly because they lead to many (costly)
mispredicted branches (see also below); they have been replaced by
reductions using a multiplication with a precomputed reciprocal.
...
modulus~$m$ --- all odd numbers below~$63$ and the primes in the range
considered --- the
...
second stage has a ranking of its own, since a prime
...
A curve with very many rational points tends to make $f$ a square modulo every
...
construction: if $a/b$ gives a rational point, so does $ka/kb$ for every~$k \ge 1$
...
the random curves goes into work the constants have little or no effect on. % @CLAUDE: check!
...
\begin{itemize}
  ...
  \item \emph{A third sieving stage} tests the survivors of the two
        ...
        option~\verb+-A+).
  \item \emph{Composite moduli.} Odd numbers below~$64$ sieve alongside
        the primes (the bound is given by~\texttt{RATPOINTS\_COMPOSITE\_MAX}
        in~\texttt{ratpoints.h}).
