...
with small coefficients costs, and is the way to measure what the estimate
is worth. Section~\ref{sec:sp3choice} has the formula, and \verb+-v+ prints
...
while this version was developed ran against expectation, and is worth
recording for whoever changes the program.

Integer division is slow --- some ten to forty cycles, against one for an
addition --- and the earlier versions avoided it wherever they reasonably
could. It turned out that division was never where the time went: the
integer divider is busy for less than $0.6\%$ of the cycles on every test
suite. The subtraction chains written to avoid divisions were themselves
worth two to three per cent of the running time; they have been replaced by
reductions using a multiplication with a precomputed reciprocal, which won
because it takes some twenty instructions fewer per call, not because it
avoids a division.
...
the structure this loop reads (Appendix~\ref{app:notes}).

A mispredicted branch costs fifteen to twenty cycles, the time of as many
\texttt{AND}s, and a branch that depends on the data mispredicts often. At
small height bounds version~2.2.4 spent more of its time on mispredicted
branches than on anything else: in the Jacobi symbol computed by a binary
algorithm for every denominator, in the gcd, in the test on each surviving
bit, in the padding of the tail of each bit array and in the bookkeeping of
the scan over the arrays. Each of these is now done without a
data-dependent branch: the Jacobi symbol is a product of Legendre symbols
read from tables built once per curve (Section~\ref{sec:elim}); the gcd
that tests a candidate for lowest terms is a binary gcd without a branch on
the data; the survivors of a word are located by counting trailing zeros;
the tail of an array is sieved in legs of 8, 4, 2 and~1 words instead of
being padded; and the scan runs into a sentinel instead of testing a
counter. The number of mispredicted branches is between $3\%$ and $40\%$ of
what version~2.2.4 had, depending on the test suite.

Two smaller lessons are of the same kind. A chain of dependent operations
sets a floor of its own: a loop that computes $x \mathrel{+}= d$; if
$x \ge p$ then $x \mathrel{-}= p$ for one table row after another costs
three cycles a row whatever else it does, and the remedy is to run several
such chains at once, as the table set-up does with four rows at a time. And
in the loops that run most often --- the scan over the bit arrays, the
set-up per denominator --- a handful of instructions removed is a gain of a
few per cent, while the same instructions elsewhere cost nothing
measurable. The residue classes of~$b$ modulo the sieving moduli are
computed by multiplication with a precomputed reciprocal for that reason,
and most of the testing of `forbidden divisors' of~$b$ is done with bit
arrays like those used for sieving the numerators, a word of 64 denominators
at a time. Appendix~\ref{app:notes} has the measurements behind all of this,
including the cases in which fewer instructions meant more cycles.
...
primes, so those primes carry no information and are discarded, and the
rule needs twenty or more primes; among the thirty primes the program
examines first, about one such curve in five does not find enough. That is
...
and it is why the number of survivors a denominator brings to the third
stage ranges over a factor of $260$ between the suites --- $0.035$ on the
random curves at $16383$, $9.2$ on the point-rich ones at $2\cdot10^5$ ---
so that no fixed number of third-stage primes can serve and the rule there
is a stopping rule.
...
modulus cap. A finer point of the same kind: three conventions for the
density of a prime and of its powers were measured, and the mixed one ---
the prime approximated, the power exact --- is the best on every suite. A
